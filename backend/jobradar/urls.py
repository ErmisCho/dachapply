...
urlpatterns=[
 path('auth/csrf/', csrf), path('auth/login/', login_view), path('auth/register/', register_view), path('auth/password-reset/', password_reset_request), path('auth/password-reset/confirm/', password_reset_confirm), path('auth/logout/', logout_view), path('auth/me/', me), path('auth/friend-requests/', friend_requests), path('profile/', profile_settings),
