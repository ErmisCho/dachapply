...
urlpatterns=[
 ...
 path('jobs/bulk-create/', bulk_create_jobs), path('public/submit/', public_submit), path('prompts/generate/', generate_prompt), path('prompts/enrich/', generate_enrichment_prompt), path('prompts/combined/', generate_combined_prompt), path('prompts/bulk-links/', generate_bulk_links_prompt), path('jobs/<int:job_id>/cv-generation/', cv_generation_preview), path('jobs/<int:job_id>/cv-generation/run/', generate_cv_documents), path('jobs/<int:job_id>/cv-generation/revise-latest/', revise_latest_cv_documents), path('cv-generation/tasks/<str:task_id>/', cv_generation_status), path('cv-generation/tasks/<str:task_id>/revise/', revise_cv_documents), path('cv-generation/tasks/<str:task_id>/download/', cv_generation_download), path('evaluations/import/', import_eval), path('stats/', stats),
