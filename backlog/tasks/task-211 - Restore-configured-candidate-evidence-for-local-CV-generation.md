...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 When usable owner candidate evidence exists in the supported account or configured-file source, Generate CV preview and generation no longer report that candidate evidence is empty
- [ ] #2 The fix preserves the established evidence-source precedence and never substitutes fabricated, demo, or another user's evidence
- [ ] #3 A genuinely empty account and unavailable/empty configured source still fails closed with an actionable error
- [ ] #4 Regression tests use synthetic evidence and cover the reproduced local-runtime boundary without invoking an external model
- [ ] #5 Full backend/frontend gates and browser verification pass on the released local workflow
<!-- AC:END -->
