...
Added an exact first-statement no-change classifier and an already-ready task containing the owner's unchanged existing artifacts. Restart-recovered revisions create that ready task; in-memory revisions reuse their ready parent. Correction images and real edits retain the original generation path. Focused tests pass (3 direct, 28 CV/evidence tests), with the endpoint regression enforcing <250 ms and model/compiler sentinels proving zero invocation. Full CI and released-runtime timing remain pending.
